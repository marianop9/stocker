import type { Category, ProductAttribute, Provider } from '$lib/models/attributes.model';
import { _pbService } from '$lib/pocketbase';

abstract class ProductAttributeService<T extends ProductAttribute> {
    ...
    async update(attr: T): Promise<T> {
        return await _pbService.collection(this.collectionName).update(attr.id, attr);
    }
